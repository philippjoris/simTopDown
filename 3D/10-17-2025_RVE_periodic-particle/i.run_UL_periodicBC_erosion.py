...
mat[0] = GMat.LinearHardeningDamage2d(
    ...
    tauy0=np.ones([nelem[0], nipBulk])*5.0,
...
mat[1] = GMat.LinearHardeningDamage2d(
    ...
    tauy0=np.ones([nelem[1], nipBulk])*1.0,
...
mat[2] = GMat.LinearHardeningDamage2d(
    ...
    D1=np.ones([nelem[2], nipBulk])*0.05,
    ...
    if converged:
        for i in range(nr_materials):
            ...
            if failure:  
                ...
                elem_failed_prev[i].update(elem_failed)
                to_be_deleted[i].extend(list(newly_failed))

                print(f"INFO: Element(s) failed of material {i}: {to_be_deleted[i]}.")

                while to_be_deleted[i]:
...
if args.plot:
    ...
    pf.plot_3d(plot_data, "failed_elements", args)