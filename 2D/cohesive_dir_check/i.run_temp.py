...
matCohPart = GooseFEM.ConstitutiveModels.CohesiveBilinear2d(
    ...
    deltafrac=np.ones([nelemCohPart, nipCohPart])*0.4,
...
for ilam, lam in enumerate(np.linspace(0.0, 1.0, ninc)):
    ...
    # du.fill(0.0)
    # update displacement
    disp[mesh["TopLine"], 1] += (+0.3/ninc)
    disp[mesh["ParticleLine"], 0] = 0.0  
    disp[mesh["ParticleLine"], 1] = 0.0 
    disp[mesh["BottomLine"], 0] = 0.0  # not strictly needed: default == 0
    disp[mesh["BottomLine"], 1] = 0.0  # not strictly needed: default == 0    
