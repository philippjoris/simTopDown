...
matCohPart = GooseFEM.ConstitutiveModels.CohesiveBilinear2d(
    Kn=np.ones([nelemCohPart, nipCohPart])*3030.0,
    Kt=np.ones([nelemCohPart, nipCohPart])*3030.0,
...
for ilam, lam in enumerate(np.linspace(0.0, 1.0, ninc)):
    ...
    # du.fill(0.0)
    # update displacement
    disp[mesh["LeftUpperLine"], 1] += (+0.15/ninc)
    disp[mesh["LeftLowerLine"], 1] += (-0.15/ninc)
