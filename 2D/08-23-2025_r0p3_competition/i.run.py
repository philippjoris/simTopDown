...
from srcTopDown.helper_functions.gmsh.parser_2D_frag import parse_msh

#
# Example with 2D geometry with one particle and horizontal cohesive zone
#

# mesh
# ----
mesh_file_name = "i.geo_simple_fragment.msh2"
...
matCohPart = GooseFEM.ConstitutiveModels.CohesiveBilinear2d(
    Kn=np.ones([nelemCohPart, nipCohPart])*15.0,
    Kt=np.ones([nelemCohPart, nipCohPart])*15.0,
    delta0=np.ones([nelemCohPart, nipCohPart])*0.02,
    deltafrac=np.ones([nelemCohPart, nipCohPart])*0.9,
...
for ilam, lam in enumerate(np.linspace(0.0, 1.0, ninc)):
    ...
    # du.fill(0.0)
    # update displacement
    disp[mesh["LeftUpperLine"], 1] += (+0.2/ninc)
    disp[mesh["LeftLowerLine"], 1] += (-0.2/ninc)
...
if args.plot:
    ...
    # optional save
    if args.save:
        fig.savefig('fixed-disp_damage_50prc-particle-bond.pdf')
    else:
        plt.show()
