...
while particles_placed < N_particles_needed:
    ...
    while attempt < MAX_PLACEMENT_ATTEMPTS:
        ...
        new_bbox = (x, y, z, x + L_PARTICLE, y + L_PARTICLE, z + L_PARTICLE)

        # Simple overlap check on coordinates
        is_overlapping = False
        for old_bbox in particle_bboxes:
            # Check for non-intersection
            if (new_bbox[3] <= old_bbox[0] or new_bbox[0] >= old_bbox[3] or
                new_bbox[4] <= old_bbox[1] or new_bbox[1] >= old_bbox[4] or
                new_bbox[5] <= old_bbox[2] or new_bbox[2] >= old_bbox[5]):
                continue
            else:
                is_overlapping = True
                break
        ...
        if not is_overlapping:
            # ONLY STORE THE BBOX, DO NOT CREATE PARTICLE GEOMETRY (will be removed later)
            particle_bboxes.append(new_bbox)
            particles_placed += 1
...
for i in range(num_elements):
    ...
    start_idx = i * num_nodes_per_element
    ...
    node_dict = {tag: all_coords[3*i:3*i+3] for i, tag in enumerate(all_tags)}
    ...
    # Check if the COM falls within the bounding box of any placed particle
    for x_start, y_start, z_start, x_end, y_end, z_end in particle_bboxes:
        if (x_start - 1e-9 <= com_x < x_end + 1e-9 and
            y_start - 1e-9 <= com_y < y_end + 1e-9 and
            z_start - 1e-9 <= com_z < z_end + 1e-9):
        ...
        if (x_start - t_interface <= com_x <= x_end + t_interface and
            ...
            if not (x_start <= com_x <= x_end and
    ...
    if is_particle_element:
        ...
        interface_elem_tags.append(element_tags[0][i])
    else:
        matrix_elem_tags.append(element_tags[0][i])
